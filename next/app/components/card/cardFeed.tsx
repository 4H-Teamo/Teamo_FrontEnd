...
const CardLayout =({type,data}:CardLayoutProps)=>{
	...
	const handleClick = () => {
		if (type === "team") {
			...
			router.push(`/detail/${type}/${post.postId}`);
		} else {
			const user= data as User;
			router.push(`/detail/${type}/${user.userId}`);
		}
	};
	return (
		<div className="max-w-80 h-[22rem] border rounded-xl border-gray-300 flex flex-col p-4 font-semibold text-sm mt-8" 	onClick={handleClick}>
